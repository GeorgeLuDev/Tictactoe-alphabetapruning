...
def ai_turn():
    if (moves.get(turn_to_letters()) != None):
        # print("already been calculated")
        cachedresult = moves.get(turn_to_letters())
        x,y = cachedresult // game.board_size, cachedresult % game.board_size
        return x,y
    bestScore = float('-inf')
    x, y = None, None
    depth = 9
    ...
    for i in range(game.board_size):
        for j in range(game.board_size):
            if (game.board[i][j] == 0):
                game.board[i][j] = 1
                score = minimax(depth - 1, alphabeta, False)
                game.board[i][j] = 0
                if (score > bestScore):
                    bestScore = score
                    x,y = i,j
    if (moves.get(turn_to_letters()) == None):
        ...
        moves[turn_to_letters()] = x*3+y
    return x,y

def minimax(depth, alphabeta, maximizingPlayer):
    global counter
    if depth == 0:
        return 0
        ...
        for i in range(game.board_size):
            for j in range(game.board_size):
                if (game.board[i][j] == 0):
                    ...
                    if (maxScore >= alphabeta[1]):
                        return maxScore

                    alphabeta[0] = max(alphabeta[0],maxScore)
        moves[turn_to_letters()] = x*3+y
        return maxScore
        ...
        for i in range(game.board_size):
            for j in range(game.board_size):
                if (game.board[i][j] == 0):
                    counter += 1
                    game.board[i][j] = 2
                    score = minimax(depth - 1, alphabeta, True)
                    game.board[i][j] = 0
                    minScore = min(minScore, score)
                    if (minScore <= alphabeta[0]):
                        return minScore
                    
                    alphabeta[1] = min(alphabeta[1],minScore)
...
print("scores")
print("wins",ai_win)
print("loses",human_win)
print("ties",tie)

print("counter", counter)
print()
print("len of dictionary",len(moves))

json.dump(moves,open("policy.txt","w"))